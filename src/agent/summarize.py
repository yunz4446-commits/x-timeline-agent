...
SUMMARIZE_PROMPT = """你是内容策展人。请阅读以下X推文，写一份深度总结。
...
## 核心要求
你的任务是像人类刷X一样，发现哪些话题/事件/人物/观点被反复提及，并把不同人的观点串起来。

### 发现热点话题
- 扫描全部推文，统计哪些话题、事件、人名、项目反复出现
...
- 不同人之间是否有共识或分歧
- 如果有影响力的账号表态，明确指出是支持还是反对

### 交叉对比
- 标注观点之间的关系："A和B都支持X" / "C和D对X持相反看法，C认为...D认为..."
- 如果某话题在上轮总结或历史快照中已出现，说明是"延续"还是"变化"
...
{{
  "overview": "总览：过去一段时间整体在聊什么（2-3句中文，具体到事件/人名/话题）",
  "topics": [
    {{
      "topic": "话题名（具体到事件/人名/项目，不要用抽象类别）",
      "mention_count": 提及次数,
      "is_continuation": true或false,
      "sentiment": "积极/消极/分歧/中性",
      "perspectives": [
        {{"author": "作者用户名（不带@）", "angle": "具体观点（引用原文关键信息）", "tweet_id": "推文ID", "time": "HH:MM"}}
      ],
      "cross_ref": "这个topic内不同观点的关系描述，如：xxx和xxx都支持，但xxx认为问题在于..."
    }}
  ],
  "sentiment": "积极/消极/分歧/中性",
  "tweet_count": 总推文数,
...
注意：
- 不要用"某个话题""某项目""某人"等模糊表述，必须写出具体名字
- perspectives 中每一条必须来自一条具体的推文，tweet_id 必须准确
- 仅客观转述推文内容，不添加主观判断。内容仅供参考。"""

# 独立总结 prompt（无上轮上下文，用于定时推送等固定窗口场景）
STANDALONE_PROMPT = """你是内容策展人。请阅读以下X推文，写一份深度总结。
...
返回纯 JSON（不要 markdown）：
{{
  "overview": "总览：这段时间整体在聊什么（2-3句中文，具体到事件/人名/话题）",
  "topics": [
    {{
      "topic": "话题名（具体到事件/人名/项目）",
      "mention_count": 提及次数,
      "sentiment": "积极/消极/分歧/中性",
      "perspectives": [
        {{"author": "作者用户名（不带@）", "angle": "具体观点（引用原文关键信息）", "tweet_id": "推文ID", "time": "HH:MM"}}
...
class SummarizeManager:
    ...
    def summarize(self, llm, model: str, session: Session,
        ...
        if not result.get("ok"):
            return {"error": result.get("error", "LLM 调用失败"), "code": result.get("code", "retryable")}
