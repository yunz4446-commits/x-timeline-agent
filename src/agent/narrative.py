"""Narrative manager — persistent information memo that accumulates across queries."""
...
UPDATE_PROMPT = """你是内容策展人。你维护一份持续更新的信息备忘录。
...
1. 继承旧备忘录中仍然有效的洞察和趋势
2. 融入新推文中的新主题、新事件、新观点——具体到话题、项目名、人名
3. 如果旧主题在新推文中继续出现，描述是"延续"还是"变化"
4. 如果同一主题出现相反观点，标注分歧（谁支持 vs 谁反对）
5. 如果有热门话题、意见领袖集体表态等群体行为，指出
6. 更新整体讨论氛围

返回纯 JSON（不要 markdown）：
{{"narrative": "更新后的梗概（3-5句中文，具体到话题/事件/人名）",
 "themes": ["主题1", "主题2", "..."],
 "sentiment": "积极/消极/分歧/中性"}}"""


FIRST_PROMPT = """你是内容策展人。请基于以下推文生成一份信息备忘录。
...
1. 大家主要在聊什么？具体到话题、项目名、人名
2. 有没有反复出现的主题、意见领袖集体表态、热门话题？
3. 不同观点有没有分歧？
4. 整体讨论氛围是什么？

返回纯 JSON（不要 markdown）：
{{"narrative": "梗概（3-5句中文，具体到话题/事件/人名）",
 "themes": ["主题1", "主题2", "..."],
 "sentiment": "积极/消极/分歧/中性"}}"""
